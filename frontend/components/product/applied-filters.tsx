...
export function AppliedFilters({ filters, filterOptions, onFiltersChange, onClearAll }: AppliedFiltersProps) {
  const appliedFilters: Array<{ key: string; label: string; value: string; displayValue: string }> = []
  ...
  if (filters.min_price !== undefined || filters.max_price !== undefined) {
    ...
    if (minPrice !== filterOptions.price_range.min || maxPrice !== filterOptions.price_range.max) {
      appliedFilters.push({
        key: "price",
        label: "Price Range",
        value: "price",
    ...
    switch (filterKey) {
      ...
      case "categories":
        if (newFilters.categories) {
          const categories = newFilters.categories.split(",").filter((cat) => cat !== filterValue)
          newFilters.categories = categories.length > 0 ? categories.join(",") : undefined
        }
        break
      case "brand":
        if (newFilters.brand) {
          const brands = newFilters.brand.split(",").filter((brand) => brand !== filterValue)
    ...
    <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {appliedFilters.map((filter, index) => (
          ...
          >
            <span className="font-medium text-gray-500 text-xs">{filter.label}:</span>
