...
const EditVariableProduct = () => {
  ...
  // Fetch product data for editing - এটাই মূল ফিক্স
  useEffect(() => {
    if (productId) {
      fetchProductData();
    }
  ...
  const fetchProductData = async () => {
    try {
      ...
      // প্রথমে API থেকে ডেটা আনুন
      await getVariableProductById(productId);

      // তারপর singleVariableProduct ব্যবহার করুন
      const product = singleVariableProduct;

      console.log("Fetched product data for editing:", product);
      console.log("Product brand:", product?.brand);
      console.log("Product screenSolution:", product?.screenSolution);

      if (product) {
        // Brand ঠিকভাবে extract করুন
        let brandValue = "";
        if (product.brand) {
          if (typeof product.brand === "object") {
            // যদি brand একটি object হয়
            brandValue = product.brand._id || product.brand.name || "";
          } else {
            // যদি brand একটি string ID হয়
            brandValue = product.brand;
          }
        }

        // Screen Solution ঠিকভাবে extract করুন
        let screenSolutionValue = "";
        if (product.screenSolution) {
          if (typeof product.screenSolution === "object") {
            // যদি screenSolution একটি object হয়
            screenSolutionValue =
              product.screenSolution._id || product.screenSolution.name || "";
          } else {
            // যদি screenSolution একটি string ID হয়
            screenSolutionValue = product.screenSolution;
          }
        }

        // Category ঠিকভাবে extract করুন
        let categoryValue = "";
        if (product.category) {
          if (typeof product.category === "object") {
            categoryValue = product.category._id || product.category.name || "";
          } else {
        ...
        setProductData({
          ...
          bottomContent: product.bottomContent || "",
          schemaMarkup: product.schemaMarkup || "",
          canonicalUrl: product.canonicalUrl || "",
          focusKeywords:
            product.seo?.keywords?.join(", ") ||
            product.seo?.focusKeywords?.join(", ") ||
            "",
          mainImage: product.mainImage || null,
          galleryImages: product.galleryImages || [],
          category: categoryValue,
          brand: brandValue,
          screenSolution: screenSolutionValue,
          tags: product.tags || "",
          visibility: product.status || "published",
          isActive: product.isActive !== false,
        });

        // Set main image preview if exists - FIXED
        if (product.mainImage) {
          console.log("Setting main image preview:", product.mainImage);
          // Check if it's a URL or base64 string
          if (
            // product.mainImage.startsWith("http") ||
            product.mainImage.startsWith("/uploads")
          ) {
            setMainImagePreview(api + product.mainImage);
          } else {
        ...
        // Set gallery images preview - FIXED
        if (product.galleryImages && product.galleryImages.length > 0) {
          console.log("Setting gallery images:", product.galleryImages);
          const galleryPreviews = product.galleryImages.map((img) => {
          ...
          setGalleryImagesPreview(galleryPreviews);
        }

        // Set categories - FIXED
        if (product.category) {
          console.log("Setting category:", product.category);
          if (typeof product.category === "object" && product.category.name) {
            setSelectedCategories([product.category.name]);
        ...
        // Set tags - FIXED
        if (product.tags) {
          console.log("Setting tags:", product.tags);
          let tagsArray = [];
          if (Array.isArray(product.tags)) {
            tagsArray = product.tags.map((tag) =>
              typeof tag === "object" ? tag.name : tag
            );
          } else if (typeof product.tags === "string") {
            tagsArray = product.tags.split(",").map((tag) => tag.trim());
          }
          setSelectedTags(tagsArray.map((tag) => tag.toLowerCase()));
        }

        // Set attributes from product attributes
        if (product.attributes && Array.isArray(product.attributes)) {
          console.log("Setting attributes:", product.attributes);
          const formattedAttributes = product.attributes.map((attr, index) => ({
            id: attr._id || Date.now() + index,
            name: attr.name,
            values: attr.values || [],
        ...
        // Set variations with images - FIXED
        if (product.variations && Array.isArray(product.variations)) {
          console.log("Setting variations:", product.variations);
          const formattedVariations = product.variations.map(
            (variation, index) => {
              let variationImage = null;
              if (variation.image) {
                if (
                  variation.image.startsWith("http") ||
                  variation.image.startsWith("data:image")
                ) {
                  variationImage = variation.image;
                } else {
                  variationImage = `${api}/${variation.image}`;
                }
              }

              return {
                id: variation._id || Date.now() + index,
  ...
  // Handle save function for editing
  const handleSave = async () => {
    try {
      ...
      // Main image যোগ করুন (যদি নতুন ছবি আপলোড করা হয়)
      if (mainImageFile) {
        formData.append("mainImage", mainImageFile);
      } else if (mainImagePreview && !mainImageFile) {
        // যদি existing image থাকে কিন্তু নতুন করে আপলোড না করা হয়
      ...
      if (galleryImageFiles.length > 0) {
        ...
        // যদি existing gallery images থাকে কিন্তু নতুন করে আপলোড না করা হয়
        formData.append(
          "galleryImagesUrls",
          JSON.stringify(galleryImagesPreview)
        );
      }
      ...
      console.log("Screen Solution:", productData.screenSolution);
      ...
      // updateVariableProduct ফাংশনে FormData পাঠান
      await updateVariableProduct(formData, productId);
    ...
    return (
      <div className="flex-1 p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white mt-4">Loading product data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center">
            <FaEdit className="mr-2" /> Edit Variable Product
          </h1>
          <p className="text-gray-400">
            Edit your product with variations using attributes
          </p>
          <p className="text-sm text-rose-gold mt-1">Product ID: {productId}</p>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="bg-rose-gold hover:bg-pink-600 text-white px-4 py-2 rounded-xl font-medium transition-all duration-300 flex items-center disabled:opacity-50"
          >
            {saving ? (
              <>
