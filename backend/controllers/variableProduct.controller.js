...
export const createVariableProduct = async (req, res) => {
  try {
      ...
      Object.keys(req.files).forEach((key) => {
        if (
          ...
          if (match && match[1]) {
            ...
            if (Array.isArray(fileArr) && fileArr[0]?.filename) {
    ...
    // Process attributes
    const processedAttributes = parsedAttributes.map((attr) => ({
      name: attr.name,
      values: attr.values || [],
    ...
    const newProduct = new VariableProduct({
      ...
      brand: brand || null,
      tags: parsedTags.map((t) => t.toLowerCase().trim()),
      screenSolution,
      seo: parsedSeo
        ? {
...
export const deleteVariableProduct = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    return res.status(200).json({
      success: true,
      message: "Variable product deleted successfully along with all images",
    });
  } catch (error) {
